...
class RepositoryGenerator(Generator):
    ...
    def generate_base_repository_model(self, model):
        self.main_generator.generate("backend/app/repository/repository_template.scala",
    ...
    def generate_repository_full_model(self, model):
        for i in range(len(model.products)):
            self.main_generator.generate("backend/app/repository/repository_full_template.scala",
                     os.path.join("backend", "app", model.project.package.name.replace(".", os.sep), "repository"),
    ...
    def generate_maybe_filter_model(self, model):
        self.main_generator.generate("backend/app/search/filter/maybe_filter_template.scala",
                     os.path.join("backend", "app", model.project.package.name.replace(".", os.sep), "search", "filter"),
                     "MaybeFilter.scala",
