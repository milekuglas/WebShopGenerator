...
class ServiceGenerator(Generator):
    ...
    def generate(self, model):
        ...
        self.generate_user_service_model(model)
        self.generate_auth_service_model(model)
    ...
    def generate_auth_service_model(self, model):
        self.main_generator.generate(
            "backend/app/service/service_auth_template.scala",
            os.path.join("backend", "app", model.project.package.name.replace(".", os.sep), "service"),
            "AuthService.scala",
            {"package": model.project.package}
        )

    def generate_user_service_model(self, model):
        self.main_generator.generate(
            "backend/app/service/service_user_template.scala",
            os.path.join("backend", "app", model.project.package.name.replace(".", os.sep), "service"),
            "UserService.scala",
            {"package": model.project.package}
        )
