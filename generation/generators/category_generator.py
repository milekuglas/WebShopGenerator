...
class CategoryGenerator(Generator):
    ...
    def generate_case_class_model(self, model):
        self.main_generator.generate("case_class_category_template.scala",
                 os.path.join("app", model.project.package.name.replace(".", os.sep), "model"),
                 "Category.scala",
                 {"package": model.project.package})

    def generate_table_model(self, model):
        self.main_generator.generate("repository_table_category_template.scala",
                 os.path.join("app", model.project.package.name.replace(".", os.sep), "repository", "table"),
                 "CategoryTable.scala",
                 {"package": model.project.package})
    
    def generate_repository_model(self, model):
        self.main_generator.generate("repository_category_template.scala",
    ...
    def generate_service_model(self, model):
        self.main_generator.generate("service_category_template.scala",
                os.path.join("app", model.project.package.name.replace(".", os.sep), "service"),
                "CategoryService.scala",
                {"package": model.project.package})

    def generate_dto_model(self, model):
        self.main_generator.generate("dto_get_category_template.scala",
                os.path.join("app", model.project.package.name.replace(".", os.sep), "dto"),
                "GetCategory.scala",
                {"package": model.project.package})
        self.main_generator.generate("dto_post_category_template.scala",
                os.path.join("app", model.project.package.name.replace(".", os.sep), "dto"),
                "PostCategory.scala",
                {"package": model.project.package})
    
    def generate_controller_model(self, model):
        self.main_generator.generate("controller_category_template.scala",
                os.path.join("app", model.project.package.name.replace(".", os.sep), "controller"),
